...
void rainResponse(GeometryOutput input, float3 lightVector, float lightIntensity, float3 lightColor, float3 eyeVector, bool fallOffFactor, inout float4 rainResponseVal)
{
    float opacity = 0.0;
    ...
    if (fallOff > 0.01 && lightIntensity > 0.01)
    {
        ...
        float col1 = Map.Sample(Sampler, tex1) * g_rainfactors[texIndicesV1.x];
        float col2 = Map.Sample(Sampler, tex2) * g_rainfactors[texIndicesV1.y];
        float col3 = Map.Sample(Sampler, tex3) * g_rainfactors[texIndicesV2.x];
        float col4 = Map.Sample(Sampler, tex4) * g_rainfactors[texIndicesV2.y];
...
{
    float3 uvw = float3(input.Uv, input.TextureId % 370);
    ...
    rainResponse(input, LightDirection, 2.0 * 1.0f * 1.0 * input.Random, float3(1.0, 1.0, 1.0), ViewPosition, false, directionalLight);
