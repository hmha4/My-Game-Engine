...
EditorManager::EditorManager()
	: sky(NULL), shadow(NULL), lake(NULL)
...
void EditorManager::Ready()
{
	...
	ParticleManager::PTinfo desc;
	desc.velocity = 3.830f;
	desc.velocityVar = 0;
	desc.gravity = 0;
	desc.size = D3DXVECTOR2(1, 1);
	desc.startSize = D3DXVECTOR2(2, 2);
	desc.endSize = D3DXVECTOR2(1, 1);
	desc.particleCount = 200;
	desc.perSecontNum = 200;
	desc.lifeTime = 1;
	desc.startRange = 0.66f;
	desc.animSpeed = 16.0f;
	for (int i = 0; i < 16; i++)
		desc.textures.push_back(Textures + L"Fire/Fire" + to_wstring(i) + L".png");
	desc.type = ParticleSystem::ParticleType::Cylinder;
	desc.option = ParticleSystem::ParticleOption::Repeat;
	
	ParticleManager::Get()->InputParticle(L"Fire", 1, desc);
	ParticleManager::Get()->Shot(L"Fire", D3DXVECTOR3(28.157f, 4.6f, 33.537f), D3DXVECTOR3(0, 1, 0));
}
