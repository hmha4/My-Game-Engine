...
//===========================Geometry=========================//
Geometry::Geometry(XYZAxis axis, int TransModel)
...
{
	shader = new Shader(Effects + L"001_Gizmo.hlsl");
...
bool Geometry::IsPicked()
{
	...
	Context::Get()->GetViewport()->GetRay(&ray, start,world,view, perspective);
...
void Geometry::BoxInit()
{
	if (axisType == XYZAxis::XBox)
	{
		...
		D3DXMatrixIdentity(&rootAxis);
		D3DXMATRIX S,T;
		D3DXMatrixScaling(&S, 3, 0.4f, 0.4f);
		D3DXMatrixTranslation(&T, 3.5f, 0, 0);
		rootAxis = S*T;
	}
	else if (axisType == XYZAxis::YBox)
	{
		SetColor(0, 1, 0);
		D3DXMATRIX S, T;
		D3DXMatrixScaling(&S, 0.4f, 3, 0.4f);
		D3DXMatrixTranslation(&T, 0, 3.5f, 0);
		rootAxis = S*T;
	}
	else if (axisType == XYZAxis::ZBox)
	{
		SetColor(0, 0, 1);
		D3DXMATRIX S, T;
		D3DXMatrixScaling(&S, 0.4f, 0.4f, 3);
		D3DXMatrixTranslation(&T, 0, 0, 3.5f);
		rootAxis = S*T;
	}
}
...
void Geometry::Render()
{
	...
	else if(axisType == XYZAxis::YZPlane || axisType == XYZAxis::XYPlane || axisType == XYZAxis::XZPlane) // 면일때
	{
		...
		shader->AsMatrix("World")->SetMatrix(world);
		shader->AsVector("Color")->SetFloatVector(gizmoColor);
		shader->AsScalar("IsPick")->SetBool(pickState);
...
//===========================Geometry_Scale=========================//
Geometry_Scale::Geometry_Scale(XYZAxis axis, GizmoType gizmoType)
	:Geometry(axis)
{
...
//===========================Geometry_Rotate=========================//
Geometry_Rotate::Geometry_Rotate(XYZAxis axisType, GizmoType gizmoType)
	:Geometry(axisType)
{
	geoType = GeometryType::AXIS_TYPE;
	this->gizmoType = gizmoType;
...
//===========================Geometry_Trans=========================//
Geometry_Trans::Geometry_Trans(XYZAxis axis, GizmoType gizmoType)
	:Geometry(axis)
{
	geoType = GeometryType::AXIS_TYPE;
	this->gizmoType = gizmoType;
...
Geometry_Plane::Geometry_Plane(XYZAxis axis, GizmoType gizmoType)
	:Geometry(axis, 0)
{
...
Geometry_Line::Geometry_Line(XYZAxis axis, GizmoType gizmoType)
	:Geometry(axis, 0)
{
	geoType = GeometryType::GEO_TYPE;
	this->gizmoType = gizmoType;
...
Geometry_Box::Geometry_Box(XYZAxis axis, GizmoType gizmoType)
	:Geometry(axis,0)
{
	geoType = GeometryType::GEO_TYPE;
	this->gizmoType = gizmoType;
