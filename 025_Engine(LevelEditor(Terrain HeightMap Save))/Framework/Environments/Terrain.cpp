...
{
	desc.material->GetEffect()->AsScalar("LineType")->SetInt(tline.LineType);
	desc.material->GetEffect()->AsVector("LineColor")->SetFloatVector(tline.LineColor);
	desc.material->GetEffect()->AsScalar("Spacing")->SetInt(tline.Spacing);
	desc.material->GetEffect()->AsScalar("Thickness")->SetFloat(tline.Thickness);
}

void Terrain::SaveHeightMap(Effect * effect, ID3D11Buffer ** buffer, ID3D11UnorderedAccessView ** uav)
{
	float * dest = new float[desc.HeightMapWidth * desc.HeightMapHeight];
	CsResource::CreateStructuredBuffer
	(
		sizeof(float), desc.HeightMapWidth * desc.HeightMapHeight,
		dest, buffer, false, true
	);

	CsResource::CreateUAV(*buffer, uav);

	effect->AsShaderResource("Input")->SetResource(render->GetHeightMapSRV());
	...
	{
		memcpy(heightMap->Datas(), resource.pData, sizeof(float) * desc.HeightMapWidth * desc.HeightMapHeight);
	}
...
{
	desc.material->GetEffect()->AsMatrix("LightView")->SetMatrix(v);
	desc.material->GetEffect()->AsMatrix("LightProjection")->SetMatrix(p);
}

void Terrain::SetEffect(D3DXMATRIX shadowTransform, ID3D11ShaderResourceView * srv)
{
	desc.material->GetEffect()->AsMatrix("ShadowTransform")->SetMatrix(shadowTransform);

	if (srv != NULL)
		desc.material->GetEffect()->AsShaderResource("ShadowMap")->SetResource(srv);
}
