...
{
	ImGui::SliderInt("Num of Flowers##Flower", (int*)&flowerDesc->FlowerNum, 1, 1000);
	...
	if (ImGui::ImageButton(flowerDesc->material->GetDiffuseMap()->SRV(), ImVec2(50, 50)))
		Path::OpenFileDialog(bind(&Flower::LoadFlowerMapFile, this, placeholders::_1));
	...
	if (bPicked)
	{
		...
		if (Mouse::Get()->Down(2))
		{
			GenerateFlower(position, terrain->brush.Range * 2, flowerDesc->FlowerNum);
		}
	}
...
void Flower::LoadData()
{
	for (UINT i = 0; i < flowerCount; i++)
	{
		...
		flowers[i]->material->SetDiffuseMap(flowers[i]->DiffuseMap);
