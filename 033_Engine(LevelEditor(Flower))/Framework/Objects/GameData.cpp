...
GameDataSpec * GameDataSpecManager::LoadTerrain(wstring file)
{
	...
	for (UINT i = 0; i < spec->FlowerCount; i++)
	{
		...
		spec->flowerDesc[i].FlowerDiffuseMap = String::ToWString(flower->GetText());
...
void GameDataSpecManager::WriteTerrain(wstring fileName, GameTerrainSpec * spec)
{
	...
	for (UINT i = 0; i < spec->FlowerCount; i++)
	{
		...
		flower = document->NewElement("FlowerDiffuseMap");
		flower->SetText(String::ToString(spec->flowerDesc[i].FlowerDiffuseMap).c_str());
