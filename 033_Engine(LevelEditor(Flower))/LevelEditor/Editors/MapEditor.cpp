...
MapEditor::MapEditor()
	...
	, terrain(NULL)
	, brushEffect(NULL), srv(NULL), uav(NULL)
	, saveHeightMapEffect(NULL), saveBuffer(NULL), saveUAV(NULL)
{
	for (int i = 0; i < 3; i++)
		layerImg[i] = NULL;
...
MapEditor::~MapEditor()
{
	...
	SAFE_DELETE(saveHeightMapEffect);
	SAFE_RELEASE(saveBuffer);
	SAFE_RELEASE(saveUAV);
...
{
	brushEffect = new Effect(Effects + L"023_TerrainBrush.fx", true);
	brushTypeVar = brushEffect->AsScalar("BrushType");
	brushRangeVar = brushEffect->AsScalar("BrushRange");
	brushPowerVar = brushEffect->AsScalar("BrushPower");
	pixelPositionVar = brushEffect->AsVector("PixelPosition");
	brushValueVar = brushEffect->AsScalar("BrushValue");

	saveHeightMapEffect = new Effect(Effects + L"027_SaveHeightMap.fx", true);
...
void MapEditor::Update()
{
	...
	case MapEditor::MAP_SETTYPE::SET_BRUSH_TERRAIN:
	{
		if (terrain->brush.Type != 0)
		{
			...
			if (bPicked)
			{
				terrain->BrushUpdate(position);

				if (Mouse::Get()->Press(2))
				{
					brushTypeVar->SetInt(terrain->brush.Type);
					brushRangeVar->SetInt(terrain->brush.Range);
					brushPowerVar->SetFloat(terrain->brush.BrushPower);
					pixelPositionVar->SetFloatVector(pixelPos);
					brushValueVar->SetInt((int)mBrushType);
	...
	//	Map Editor
	{
		...
		case MapEditor::MAP_SETTYPE::SET_BRUSH_TERRAIN:
		{
			...
			if (ImGui::CollapsingHeader("Brush", ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				ImGui::SliderInt("Type##Brush", &terrain->brush.Type, 0, 4);
			...
			if (ImGui::CollapsingHeader("HeightMap", ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				{
					terrain->SaveHeightMap(saveHeightMapEffect, &saveBuffer, &saveUAV);
				}
				ImGui::Separator();
				ImGui::Separator();
			}
		...
		case MapEditor::MAP_SETTYPE::SET_PLANTS_TERRAIN:
		{
			...
			if (ImGui::CollapsingHeader("Flower"))
			{
				...
				ImGui::ColorEdit3("Color##Flower", terrain->brush.Color);
				ImGui::SliderInt("Range##Flower", &terrain->brush.Range, 1, 100);
				ImGui::Spacing();

				terrain->GetFlower()->ImGuiRender();
			}
...
void MapEditor::CreateComputeTexture()
{
	...
	CsResource::CreateTexture(2049, 2049, NULL, &texture, false, true, DXGI_FORMAT_R16_FLOAT);
	CsResource::CreateSRV(texture, &srv);
	CsResource::CreateUAV(texture, &uav);
...
void MapEditor::SelectSetType()
{
	ImGui::BeginTabBar("Terrain Bars");
	{
		...
		else if (mSetType != MAP_SETTYPE::SET_BRUSH_TERRAIN)
		{
