#include "stdafx.h"
#include "TestReflreation.h"
#include "../Environment/CubeSky.h"

TestReflreation::TestReflreation()
{
	...
	bunny->SetShader(Effects + L"015_Mirror.fx");
	...
	//Create RenderTarget
	{
		...
		viewport = new Viewport((float)desc.Width, (float)desc.Height);
		rtv = new RenderTargetView(desc.Width, desc.Height);
		dsv = new DepthStencilView(desc.Width, desc.Height);
	}
...
void TestReflreation::Update()
{
	sky->Update();
	bunny->Update();
}

void TestReflreation::PreRender()
{
...
void TestReflreation::PostRender()
{
	
}

void TestReflreation::ResizeScreen()
{
}
