...
void MyGui::ShowSkySetting(ScatterSky * sky)
{
	...
	ImGui::Separator();
	{
		...
		{
			if (ImGui::ColorEdit3("Color ", (float*)sky->GetWaveLength()))
				sky->CalcWaveLength();
...
void MyGui::EditTransform(const float * cameraView, float * cameraProjection, float * matrix)
{
	...
	ImGui::DragFloat3("Position", matrixTranslation, 0.1f, -FLT_MAX, FLT_MAX);
	ImGui::DragFloat3("Rotation", matrixRotation, 0.1f, -FLT_MAX, FLT_MAX);
	ImGui::DragFloat3("Scale", matrixScale, 0.1f, 1, FLT_MAX);
