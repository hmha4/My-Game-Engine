...
void MyGui::ChangeTexture(wstring fileName, ScatterSky * sky, UINT type)
{
	...
	case 0:
	{
		...
		sky->GetShader()->AsShaderResource("StarfieldMap")->SetResource(sky->GetStarFieldMap()->SRV());
	}
	break;
	case 1:
	{
		...
		sky->GetShader()->AsShaderResource("MoonMap")->SetResource(sky->GetMoonMap()->SRV());
	}
	break;
	case 2:
	{
		...
		sky->GetShader()->AsShaderResource("MoonGlowMap")->SetResource(sky->GetGlowMap()->SRV());
	}
	break;
	case 3:
	{
		...
		sky->GetCloudMap()->WritePixels(DXGI_FORMAT_R8G8B8A8_UNORM, pixels);
		sky->GetShader()->AsShaderResource("CloudMap")->SetResource(sky->GetCloudMap()->SRV());
	}
	break;
	}
