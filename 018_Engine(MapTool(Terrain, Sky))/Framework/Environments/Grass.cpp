...
Grass::Grass(UINT terrainSize, class Terrain * terrain, UINT pathSize, UINT rootInPatchSize)
	: terrainSize(terrainSize), terrain(terrain)
{
	...
	GenerateGrass(pathSize, rootInPatchSize);
	GenerateFlower();
}
...
void Grass::GenerateGrass(UINT numberOfPatchRows, UINT numberOfRootsInPatch)
{
	...
	for (UINT x = 0; x < numOfPatchRows; x++)
	{
		for (UINT y = 0; y < numOfPatchRows; y++)
		{
...
int Grass::GeneratePatch(D3DXVECTOR3 startPos, D3DXVECTOR3 patchSize, int currentVertex)
{
	for (UINT i = 0; i < numOfRootsInPatch; i++)
	{
		...
		currentPos.y = terrain->Data((UINT)currentPos.x, (UINT)currentPos.z);
...
void Grass::DrawGrass(UINT tech, UINT pass)
{
	...
	shader->Draw(tech, pass, numOfRoots);
}
...
void Grass::GenerateFlower()
{
	...
	for (int i = 0; i < 1000; i++)
	{
		...
		position.x = Math::Random(0.0f, (float)terrainSize);
		position.z = Math::Random(0.0f, (float)terrainSize);
		position.y = terrain->Data((UINT)position.x, (UINT)position.z);
...
void Grass::DrawFlower(UINT tech, UINT pass)
{
	...
	shader->Draw(tech, pass, 1000);
}

void Grass::SetShader(D3DXMATRIX v, D3DXMATRIX p)
{
	shader->AsMatrix("LightView")->SetMatrix(v);
	shader->AsMatrix("LightProjection")->SetMatrix(p);
}

void Grass::SetShader(D3DXMATRIX shadowTransform, ID3D11ShaderResourceView * srv)
{
	shader->AsMatrix("ShadowTransform")->SetMatrix(shadowTransform);

	if (srv != NULL)
		shader->AsShaderResource("ShadowMap")->SetResource(srv);
}
