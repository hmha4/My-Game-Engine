...
void GameAnimator::BlendTransform()
{
	...
	if (time <= 1)
	{
		instance->UpdateBlending(instanceID, currentKeyframe, 0, time, clipQueue.back()->ClipIndex());

		for (UINT i = 0; i < model->BoneCount(); i++)
		{
			ModelBone*bone = model->BoneByIndex(i);

			wstring index = bone->Name();
			...
			if (currentKeyframe > curClipFrame->Transforms.size() - 1)
				currentKeyframe = curClipFrame->Transforms.size() - 1;
			...
			D3DXMatrixTranslation(&T, t.x, t.y, 0);
...
void GameAnimator::UpdateTransforms()
{
	for (UINT i = 0; i < model->BoneCount(); i++)
	{
		...
		D3DXMATRIX transform;
		ModelKeyframe * frame = clips[currentClipName]->Keyframe(i);
