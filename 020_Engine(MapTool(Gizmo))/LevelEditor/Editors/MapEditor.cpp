	...
	//	Map Editor
	{
		...
		case MapEditor::MAP_SETTYPE::SET_PLANTS_TERRAIN:
		{
			if (ImGui::CollapsingHeader("Grass", ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				static int patchRows = 1;
				static int rootsInPatch = 1;

				ImGui::SliderInt("PatchRows##Grass", &patchRows, 1, 100);
				ImGui::SliderInt("RootsInPatch##Grass", &rootsInPatch, 1, 100);
				ImGui::DragFloat2("Velocity", (float*)&terrain->GetGrass()->GetWind()->Velocity(), 0.01f, -3, 3);
				ImGui::Spacing();
				...
				ImGui::Checkbox("", &terrain->GetGrass()->GetDesc()->isFlower);
				ImGui::SameLine();
				ImGui::Text("Flower"); ImGui::NextColumn();

				if (ImGui::ImageButton(terrain->GetGrass()->GetDesc()->materialMap.diffuseMap->SRV(), ImVec2(50, 50)))
					Path::OpenFileDialog(bind(&MapEditor::LoadGrassMapFile, this, placeholders::_1, 0));
				ImGui::NextColumn();
				
				if (terrain->GetGrass()->GetDesc()->isFlower == true)
				{
					if (ImGui::ImageButton(terrain->GetGrass()->GetDesc()->materialMap.detailMap->SRV(), ImVec2(50, 50)))
						Path::OpenFileDialog(bind(&MapEditor::LoadGrassMapFile, this, placeholders::_1, 1));
					
					ImGui::Text("Count"); 
					ImGui::SliderInt("", (int*)&terrain->GetGrass()->GetDesc()->flowerNum, 1, patchRows * patchRows * rootsInPatch);
				}
				ImGui::Columns(1);
				...
				if (Mouse::Get()->Down(2))
				{
					terrain->GetGrass()->GenerateGrass(position, terrain->brush.Range * 2, patchRows, rootsInPatch);
				}
...
void MapEditor::LoadGrassMapFile(wstring filename, int index)
{
	terrain->GetGrass()->GetDesc()->ChangeTexture(filename, index);
}
