...
UINT Texture::ReadPixel(ID3D11Texture2D * src, const UINT & xPos, const UINT & yPos, DXGI_FORMAT format)
{
	D3D11_BOX srcBox;
	srcBox.left = xPos;
	...
	SAFE_RELEASE(p1x1StagingTexture);

	return pixel;
}

D3DXVECTOR4 Texture::ReadPixel128(ID3D11Texture2D * src, const UINT & xPos, const UINT & yPos)
{
	D3D11_BOX srcBox;
	srcBox.left = xPos;
	srcBox.right = srcBox.left + 1;
	...
	D3DXVECTOR4 pixel;
	D3D::GetDC()->Map(p1x1StagingTexture, 0, D3D11_MAP_READ, 0, &map);
	memcpy(&pixel, map.pData, sizeof(D3DXVECTOR4));
	D3D::GetDC()->Unmap(p1x1StagingTexture, 0);
	SAFE_RELEASE(p1x1StagingTexture);
...
void Textures::DeleteDesc(wstring fileName)
{
	vector<TextureDesc>::iterator viter;

	viter = descs.begin();

	for (; viter != descs.end();)
	{
		if (fileName == viter->file)
		{
			SAFE_RELEASE(viter->view);
			descs.erase(viter);
			break;
		}
		else
...
void Textures::Load(Texture * texture, D3DX11_IMAGE_LOAD_INFO * loadInfo)
{
	...
	else
	{
		...
		hr = DirectX::CreateShaderResourceView(
			D3D::GetDevice(), image.GetImages(), image.GetImageCount(), metaData, &view);
		assert(SUCCEEDED(hr));
...
TextureArray::TextureArray(vector<ID3D11Texture2D *>& textures, UINT width, UINT height, UINT mipLevels)
{
	D3D11_TEXTURE2D_DESC textureDesc;
	textures[0]->GetDesc(&textureDesc);
