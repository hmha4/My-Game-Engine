...
int GamePlayer::ActionSkill()
{
	...
	case 0:
		...
		break;
	case 1:
		if (model->GetClip("Skill1")->IsPlay() == true)
		{
			bSkill = true;
		...
		break;
	case 2:
		if (model->GetClip("Skill2")->IsPlay() == true)
		{
			bSkill = true;
		...
		break;
	case 3:
		if (model->GetClip("Skill3")->IsPlay() == true)
		{
			bSkill = true;
		}
		else
		...
		break;
	case 4:
		if (model->GetClip("Skill4")->IsPlay() == true)
		{
			bSkill = true;
		}
		else
	...
	else
	{
		...
		case 0:
			if (model->GetClip("Attack1")->IsPlay() == true)
			{
				...
				int currKeyframe = model->CurrKeyframe();
				if (currKeyframe > (int)model->GetClip("Attack1")->FrameCount())
					currKeyframe = 0;
			...
			else if(model->GetClip("Attack2")->IsPlay() == true)
			{
				bAttack = true;

				int currKeyframe = model->CurrKeyframe();
				int c = model->GetClip("Attack2")->FrameCount();
				if (currKeyframe > (int)model->GetClip("Attack2")->FrameCount())
					currKeyframe = 0;
