...
TrailRenderer::TrailRenderer(GameModel * model, Effect * effect)
	...
	, run(false), doSlice(false), count(0), lifeTime(0.0f), sliceAmount(-1.0f)
	...
	, vertexBuffer(NULL), indexBuffer(NULL)
{
	...
	sliceMap = new Texture(Textures + L"uniformclouds-1.jpg");

	trailMapVar = this->effect->AsSRV("TrailMap");
	alphaMapVar = this->effect->AsSRV("AlphaMap");
	this->effect->AsSRV("SliceGuide")->SetResource(sliceMap->SRV());
