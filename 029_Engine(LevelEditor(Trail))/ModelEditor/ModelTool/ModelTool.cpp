...
void ModelTool::InitTrail()
{
	...
	TrailRenderer * trail = new TrailRenderer(currGameModel, trailEffect);
	trail->Ready();
...
void ModelTool::Save()
{
	for (string name : objNames)
	{
		...
		spec->TrailCount = temp2->GetTrails().size();
		for (TrailRenderer * t : temp2->GetTrails())
		{
			GameCharacterSpec::Trail trail;
			trail.BoneIndex = t->GetBone();
			trail.LifeTime = t->GetLifeTime();
			trail.DiffuseMap = t->GetDiffuseMap();
			trail.AlphaMap = t->GetAlphaMap();
			trail.StartMat = t->GetStartMat();
			trail.EndMat = t->GetEndMat();

			spec->Trails.push_back(trail);
		}
...
void ModelTool::Load(wstring fileName)
{
	...
	for (UINT i = 0; i < spec->TrailCount; i++)
	{
		...
		trail->Ready();
		trail->SetBone(spec->Trails[i].BoneIndex);
		trail->SetLifeTime(spec->Trails[i].LifeTime);
		trail->SetDiffuseMap(spec->Trails[i].DiffuseMap);
		trail->SetAlphaMap(spec->Trails[i].AlphaMap);
		trail->SetStartMat(spec->Trails[i].StartMat);
		trail->SetEndMat(spec->Trails[i].EndMat);
