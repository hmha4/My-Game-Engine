...
void Mesh::SetShader(Shader * shader, D3DXMATRIX v, D3DXMATRIX p, D3DXMATRIX s, ID3D11ShaderResourceView * srv)
{
	GetMaterial()->SetShader(shader);

	if (shader->GetFile().find(L"023_BuildShadowMap") != wstring::npos)
	{
		GetMaterial()->GetShader()->AsMatrix("V")->SetMatrix(v);
		GetMaterial()->GetShader()->AsMatrix("P")->SetMatrix(p);
	}
	else
	{
		GetMaterial()->GetShader()->AsMatrix("ShadowTransform")->SetMatrix(s);
