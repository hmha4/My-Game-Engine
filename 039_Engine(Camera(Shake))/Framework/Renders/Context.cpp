...
Context::Context()
{
	...
	cameras.push_back(new SmoothFollow());
...
void Context::AddPointLight(UINT index, PointLightDesc & light)
{
	pointLight.Lights[index] = light;

	for (pair<Effect *, CBuffer*> temp : pointLightMap)
		temp.second->Change();
}

void Context::RemovePointLight()
{
	pointLight.Count--;
