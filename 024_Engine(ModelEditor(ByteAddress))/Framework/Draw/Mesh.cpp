...
void Mesh::Render()
{
	...
	material->GetEffect()->AsVector("SunColor")->SetFloatVector(sunColor);
...
{
	GetMaterial()->GetEffect()->AsMatrix("LightView")->SetMatrix(v);
	GetMaterial()->GetEffect()->AsMatrix("LightProjection")->SetMatrix(p);
}

void Mesh::SetEffect(D3DXMATRIX shadowTransform, ID3D11ShaderResourceView * srv)
{
	GetMaterial()->GetEffect()->AsMatrix("ShadowTransform")->SetMatrix(shadowTransform);

	if (srv != NULL)
		GetMaterial()->GetEffect()->AsShaderResource("ShadowMap")->SetResource(srv);
}
