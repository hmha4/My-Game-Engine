...
ModelTool::ModelTool()
	...
	, modelEffect(NULL), lineEffect(NULL), trailEffect(NULL)
...
{
	modelEffect = new Effect(Effects + L"026_Model.fx");
	...
	ImGui::Begin("Hierarchy", &MyGui::show_app_hierarchy, ImGuiWindowFlags_MenuBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_HorizontalScrollbar | ImGuiWindowFlags_NoMove);
	{
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("File"))
			{
				...
				{
					Path::OpenFileDialog(bind(&ModelTool::Load, this, std::placeholders::_1));
				}
		...
		if (currObject != NULL)
		{
			...
			if (ImGui::BeginPopup("Component"))
			{
				if (ImGui::Button("Collider"))
					ImGui::OpenPopup("ColliderPopup");
				if (ImGui::Button("Trail"))
					ImGui::OpenPopup("TrailPopup");
				if (ImGui::Button("BehaviourTree"))
					ImGui::OpenPopup("BTPopup");

				if (ImGui::BeginPopup("ColliderPopup"))
				{
					wstring	boneName;
					switch (target)
					{
...
void ModelTool::FixChildObject(int index)
{
	...
	for (string name : objNames)
	{
		GameModel * obj = (GameModel*)objects[String::ToWString(name)];
	...
	if (ImGui::CollapsingHeader("ChildObject"))
	{
		for (IGameObject * objs : object)
		{
			...
			ImGui::DragFloat3(string("Position##").append(modelName).c_str(), (float*)&trans, 0.1f, -100, 100);
			ImGui::DragFloat3(string("Rotation##").append(modelName).c_str(), (float*)&rot, 0.001f, -Math::PI, Math::PI);
			ImGui::DragFloat3(string("Scale##").append(modelName).c_str(), (float*)&scale, 0.01f, -10, 10);
...
void ModelTool::RenderColliderBox(GameModel * model)
{
	for (ColliderElement * collider : model->GetColliders())
	{
		...
		if (index != -1)
		{
			...
			collider->Render(bone->World());
		}
		else
			collider->Render(model->Transformed());
	}
}

void ModelTool::FixColliderBox(int index)
{
	GameModel * model = dynamic_cast<GameModel*>(currObject);
	//	To push_back colliders if there is more than one collider
	vector<ColliderElement * > ColliderBox;
	if (model != NULL)
	{
		for (ColliderElement * line : model->GetColliders())
		{
	...
	for (size_t i = 0; i < ColliderBox.size(); i++)
	{
		...
		if (ImGui::CollapsingHeader(string("Collider##").append(to_string(i)).c_str()))
		{
			...
			ImGui::Checkbox(string("Active##").append(to_string(i)).c_str(), &inUse);
			colliderBox->IsDraw(inUse);
			//	Change the name of the collider box
			char buf[64];
			strcpy(buf, String::ToString(colliderBox->Name()).c_str());
			ImGui::InputText(string("Name##").append(to_string(i)).c_str(), buf, 64);
			if (Keyboard::Get()->Down(VK_RETURN))
			...
			ImGui::DragFloat3(string("Center##Collider").append(to_string(i)).c_str(), (float*)&position, 0.1f, -200, 200);
			ImGui::DragFloat3(string("Rotation##Collider").append(to_string(i)).c_str(), (float*)&rot, 0.1f, -200, 200);
			ImGui::DragFloat3(string("Size##Collider").append(to_string(i)).c_str(), (float*)&scale, 0.01f, -200, 200);
...
void ModelTool::FixTrail(int index)
{
	...
	for (size_t i = 0; i < render->GetTrails().size(); i++)
	{
		if (render->GetTrails()[i]->GetBone() == index)
			trails.push_back(render->GetTrails()[i]);
	}
...
{
	GameModel * render = dynamic_cast<GameModel*>(currObject);
	for (TrailRenderer * trail : render->GetTrails())
	{
		trail->SetRun(val);
	}
}
...
{
	Path::OpenFileDialog(bind(&ModelTool::ExportModel, this, placeholders::_1, type));
}
...
void ModelTool::ExportAnim(wstring file)
{
	...
	if (model != NULL)
		model->AddClip(Models + modelName + L"/" + fileName + L".anim");
}

void ModelTool::OpenAnimFile()
{
	Path::OpenFileDialog(bind(&ModelTool::ExportAnim, this, placeholders::_1));
}
...
{
	Path::OpenFileDialog(bind(&ModelTool::LoadMaterial, this, placeholders::_1));
}
...
{
	Path::OpenFileDialog(bind(&ModelTool::LoadMesh, this, placeholders::_1, type));
}
...
{
	Path::OpenFileDialog(bind(&ModelTool::LoadClip, this, placeholders::_1));
}
...
void ModelTool::Save()
{
	for (string name : objNames)
	{
		...
		spec->IsChild = temp2->IsChild();
		if(spec->IsChild == true)
...
void ModelTool::Load(wstring fileName)
{
	...
	if (spec->IsChild == true)
		model->ParentBone(model->GetModel()->BoneByIndex(spec->ParentBoneIndex));
