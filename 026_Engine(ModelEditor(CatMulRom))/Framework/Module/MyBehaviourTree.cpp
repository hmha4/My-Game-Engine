			...
			ImGui::BeginGroup(); // Lock horizontal position
			{
				...
				if (node->nType == ImGuiNode::NodeType::Condition)
				{
					...
					else if (name.find("Numeric") != string::npos)
					{
						...
						ImGui::Text("CompareType");
						ImGui::RadioButton("Less", (int*)&node->cType, 0); ImGui::SameLine();
						ImGui::RadioButton("Greater", (int*)&node->cType, 1); ImGui::SameLine();
						ImGui::RadioButton("Equal", (int*)&node->cType, 2); ImGui::SameLine();
...
void MyBehaviourTree::InputFunctions(initializer_list<wstring> && name, initializer_list<function<int(void)>>&& functions)
{
	...
	{
		functionMap.insert(make_pair((*iter), func));
		iter++;
	}
}
...
void MyBehaviourTree::GetImguiTreeNode(Node * node, int curIdx, int parentIdx, int parentSibCount)
{
	int nodeId = curIdx;
	...
	if (composite != NULL)
	{
		for (nIter = composite->GetChildren().begin(); nIter != composite->GetChildren().end(); nIter++)
		{
			...
			GetImguiTreeNode((*nIter), currentIdx + curParentSibCnt, curIdx, nextParentCount);
			currentIdx++;
		}
	}
	if (decorator != NULL)
	{
		...
		if (node != NULL)
		{
			GetImguiTreeNode(node, currentIdx + curParentSibCnt, curIdx, nextParentCount);
			currentIdx++;
		}
	}
}

void MyBehaviourTree::GetImguiLink()
{
	for (int i = 0; i < nodes.size(); i++)
	{
...
void MyBehaviourTree::SaveTree(Xml::XMLElement * parent, Xml::XMLElement * element, Xml::XMLDocument*document, Node*node)
{
	...
	if (decorator != NULL)
	{
		...
		Xml::XMLElement * childElement = NULL;
		childElement = document->NewElement(String::ToString(node->type).c_str());
...
{
	static Xml::XMLElement * readBehavior = element;
	...
	for (int i = 0; i < childCount; i++)
	{
		...
		{
			bool check = readBehavior->FloatText();

			temp = CreateTFCondition(childName, check);
		}
		else if (childName.find(L"Numeric") != wstring::npos)
		{
			int val = (int)readBehavior->FloatText();
			int cType = (int)readBehavior->FloatText();
