...
TrailRenderer::TrailRenderer(Model * model, Effect * effect)
	...
	, vertexBuffer(NULL), indexBuffer(NULL)
{
	...
	ModelBone * b = model->BoneByName("RightHand");
	boneMatrix = b->World();
...
void TrailRenderer::Update()
{
	...
	ModelBone * b = model->BoneByIndex(boneIndex);
	boneMatrix = b->World();
...
{
	if (ImGui::CollapsingHeader("Trail Renderer"))
	{
		D3DXMATRIX startMat = startPos * boneMatrix;
		D3DXMATRIX endMat = endPos * boneMatrix;
...
void TrailRenderer::PostRender()
{
	...
	//DirectWrite::RenderText(wstring(L"Start").c_str(), rect, 20.0f, L"Roboto-Regular");
	//rect = DirectWrite::Get()->Get2DPosition(endPos * boneMatrix, view, proj);
	//DirectWrite::RenderText(wstring(L"End").c_str(), rect, 20.0f, L"Roboto-Regular");
}
