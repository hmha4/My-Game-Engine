...
struct BSphere : public ILine
{
	...
	BSphere(D3DXVECTOR3& center, float radius, UINT stackCount = 20, UINT sliceCount = 20)
		: Center(center), Radius(radius), StackCount(stackCount), SliceCount(sliceCount)
	{
	}
	...
	{
		D3DXVECTOR3 temp;
		Math::Vec3Clamp(Center, box->Min, box->Max, temp);

		float single = Math::DistanceSquared(Center, temp);
		if (single <= Radius * Radius)
			return true;

		return false;
	}
	bool Intersect(BSphere * sphere)
	{
		float single = Math::DistanceSquared(Center, sphere->Center);

		float r1 = Radius;
		float r2 = sphere->Radius;

		if ((r1 * r1) + (2.0f * r1 * r2) + (r2 * r2) >= single)
			return true;
	...
	bool Intersect(Ray * ray, OUT float & result)
	{
		...
		float x = Center.x - ray->Position.x;
		float y = Center.y - ray->Position.y;
		float z = Center.z - ray->Position.z;
