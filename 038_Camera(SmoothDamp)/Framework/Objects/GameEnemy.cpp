...
void GameEnemy::Update()
{
	...
	if (distance <= 30)
	{
		...
		UIManager::Get()->GetRender2D(L"07BarBack" + to_wstring(model->InstanceID()))->Position(rect.left- 100, (desc.Height - rect.bottom));
		UIManager::Get()->GetRender2D(L"08BarFront" + to_wstring(model->InstanceID()))->Position(rect.left - 100, (desc.Height - rect.bottom));
	}
	else
	{
		UIManager::Get()->SetUiRender(L"07BarBack" + to_wstring(model->InstanceID()), false);
	...
	if (hit->IsPlay() == true)
	{
		...
		if (k > 10)
		{
			...
			if (result != NULL)
			{
				...
				UIManager::Get()->GetRender2D(L"08BarFront" + to_wstring(model->InstanceID()))->SetUV(L"x", Life(), 10);
			}
		}
