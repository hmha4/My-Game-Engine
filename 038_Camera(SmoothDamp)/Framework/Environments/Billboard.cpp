...
void Billboard::RenderReflection(D3DXMATRIX view)
{
	if (vertices.size() < 1) return;
	...
	material->GetEffect()->AsMatrix("ReflectionView")->SetMatrix(view);

	material->GetEffect()->DrawInstanced(3, 0, 1, vertices.size());
}
...
void Billboard::ShadowRender(UINT tech, UINT pass)
{
	...
	material->GetEffect()->DrawInstanced(tech, pass, 1, vertices.size());
}
