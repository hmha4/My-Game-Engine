...
void ModelTool::FixTrail(int index)
{
	...
	{
		if (currGameModel->GetTrails()[i]->BoneIndex() == index)
...
void ModelTool::Save()
{
	for (string name : objNames)
	{
		...
		for (TrailRenderer * t : temp2->GetTrails())
		{
			...
			trail.BoneIndex = t->BoneIndex();
			trail.LifeTime = t->LifeTime();
			trail.DiffuseMap = t->DiffuseMap();
			trail.AlphaMap = t->AlphaMap();
			trail.StartMat = t->StartMat();
			trail.EndMat = t->EndMat();
