...
void TerrainRenderer::Ready(Material*material)
{
	...
	heightMapVar = effect->AsSRV("HeightMap");
	layerMapVar = effect->AsSRV("LayerMapArray");
	blendMapVar = effect->AsSRV("BlendMap");
	blendPosVar = effect->AsVector("BlendPositionRate");
	activeBlendVar = effect->AsVector("ActiveBlend");
	maxHeightVar = effect->AsScalar("MaxHeight");
	maxDetailIntensityVar = effect->AsScalar("DetailIntensity");
	maxDetailValurVar = effect->AsScalar("DetailValue");
	cbTerrainVar = effect->AsConstantBuffer("CB_Terrain");

	heightMapVar->SetResource(heightMapSRV);
	layerMapVar->SetResource(layerMapArraySRV);
	blendMapVar->SetResource(blendMapSRV);
	...
	blendPosVar->SetFloatVector(terrain->Desc().blendHeight);
	activeBlendVar->SetFloatVector(terrain->Desc().activeBlend);
	maxHeightVar->SetFloat(terrain->Desc().HeightScale);
	maxDetailIntensityVar->SetFloat(terrain->detail.DetailIntensity);
	maxDetailValurVar->SetFloat(terrain->detail.DetailValue);
}
...
void TerrainRenderer::AdjustY(Effect*effect, ID3D11ShaderResourceView*srv, ID3D11UnorderedAccessView * uav)
{
...
void TerrainRenderer::Render()
{
	...
	hr = cbTerrainVar->SetConstantBuffer(cBuffer);
	assert(SUCCEEDED(hr));
	...
	material->GetWorldVariable()->SetMatrix(world);
	material->GetEffect()->DrawIndexed(0, pass, patchQuadFacesCount * 4);

	if (sky != NULL)
	{
		buffer.FogColor = sky->GetSunColor();
	}
...
void TerrainRenderer::RenderReflection(D3DXMATRIX reflectionView)
{
	/*D3D11_MAPPED_SUBRESOURCE subResource;
	HRESULT hr = D3D::GetDC()->Map
	(
...
*/
	...
	material->GetWorldVariable()->SetMatrix(world);
	material->GetEffect()->AsMatrix("ReflectionView")->SetMatrix(reflectionView);
	material->GetEffect()->DrawIndexed(3, 0, patchQuadFacesCount * 4);
}
