...
#pragma endregion
		...
		if (Mouse::Get()->Press(0))
		{
			...
			if (translationDelta != zero)
			{
				...
				{
					D3DXVECTOR3 position = entity->Position();
			...
			if (rotationDelta != zero)
			{
				for (GameModel * entity : selection)
				{
					D3DXVECTOR3 rot = entity->Rotation();
			...
			if (scaleDelta != zero)
			{
				for (GameModel * entity : selection)
				{
					D3DXVECTOR3 scale = entity->Scale();

					if (activeMode == EGizmoMode::UniformScale)
					{
						D3DXVECTOR3 value = scale * (1 + ((scaleDelta.x + scaleDelta.y + scaleDelta.z) / 3));
						entity->Scale(value);
					}
					else
					{
						D3DXVECTOR3 value = scale + scaleDelta;
						entity->Scale(value);
					}
				}
				scaleDelta = zero;
			...
			{
				entity->Update();
				targetWorld = entity->Transformed();
			}
...
void Gizmo::SetSelectionPool(class GameModel *entity)
{
...
void Gizmo::PickObject()
{
	...
	{
		GameModel * entity = selectionPool[i];

		if (entity->IsPicked())
		{
			bool b = false;
			for (GameModel * find : selection)
			{
