...
{
	vertexBufferG[0] = NULL;
	vertexBufferG[1] = NULL;
...
Grass::~Grass()
{
	...
	SAFE_RELEASE(vertexBufferG[0]);
	SAFE_RELEASE(vertexBufferG[1]);
...
void Grass::Ready()
{
	...
	desc.ByteWidth = sizeof(Vertex) * 1;
	desc.BindFlags = D3D11_BIND_VERTEX_BUFFER;

	D3D11_SUBRESOURCE_DATA data = { 0 };
	data.pSysMem = tempVert;

	HRESULT hr = D3D::GetDevice()->CreateBuffer(&desc, &data, &vertexBufferG[0]);
	assert(SUCCEEDED(hr));
}
...
void Grass::ImGuiRender()
{
	...
	if (ImGui::ImageButton(grassDesc->materialMap.diffuseMap->SRV(), ImVec2(50, 50)))
		Path::OpenFileDialog(bind(&Grass::LoadGrassMapFile, this, placeholders::_1, 0));
	ImGui::NextColumn();

	if (grassDesc->isFlower == true)
	{
		if (ImGui::ImageButton(grassDesc->materialMap.detailMap->SRV(), ImVec2(50, 50)))
...
void Grass::Delete()
{
	...
	HRESULT hr = D3D::GetDevice()->CreateBuffer(&desc, &data, &vertexBufferG[1]);
	assert(SUCCEEDED(hr));
...
void Grass::DrawGrass(UINT tech, UINT pass)
{
	...
	D3D::GetDC()->IASetVertexBuffers(0, 2, vertexBufferG, stride, offset);
	...
	for (UINT i = 0; i < grassCount; i++)
	{
		...
		material->GetShader()->DrawInstanced(tech, pass, 1, grasses[i]->numOfRoots, 0, temp);
