...
void Billboard::Ready()
{
	...
	material->ChangeMaterialDesc(&matDesc);
	material->ChangeMaterialMap(&matMap);
	...
	material->GetShader()->AsShaderResource("Map")->SetResource(billboardTexture->GetSRV());
...
void Billboard::Render()
{
	...
	material->GetShader()->DrawInstanced(0, 0, 1, vertices.size());
}

void Billboard::Update()
{
	frustum->GetPlanes(buffer.plane);
	material->GetShader()->AsVector("WorldFrustumPlanes")->SetFloatVectorArray((float*)buffer.plane, 0, 6);
}
...
void Billboard::ShadowRender(UINT tech, UINT pass)
{
	...
	material->GetShader()->DrawInstanced(1, 1, 1, vertices.size());
}	
