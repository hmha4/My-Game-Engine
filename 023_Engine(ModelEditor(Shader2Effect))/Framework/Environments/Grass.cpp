...
void Grass::Update()
{
	...
	material->GetEffect()->AsVector("FrustumPlanes")->SetFloatVectorArray((float*)frustumPlane, 0, 6);
}
...
void Grass::DrawGrass(UINT tech, UINT pass)
{
	...
	for (UINT i = 0; i < grassCount; i++)
	{
		material->GetEffect()->Draw(tech, pass, grasses[i]->numOfRoots, temp);
...
void Grass::DrawFlower(UINT tech, UINT pass)
{
	...
	for (UINT i = 0; i < grassCount; i++)
	{
		...
		material->GetEffect()->Draw(tech, pass, grasses[i]->flowerNum, offset);
		offset += grasses[i]->flowerNum;
	}
}

void Grass::SetEffect(D3DXMATRIX v, D3DXMATRIX p)
{
	material->GetEffect()->AsMatrix("LightView")->SetMatrix(v);
	material->GetEffect()->AsMatrix("LightProjection")->SetMatrix(p);
}

void Grass::SetEffect(D3DXMATRIX shadowTransform, ID3D11ShaderResourceView * srv)
{
