...
void ScatterSky::Ready()
{
	...
	effect->AsShaderResource("RayleighMap")->SetResource(rayleighTarget->SRV());
	effect->AsShaderResource("MieMap")->SetResource(mieTarget->SRV());
	CalcWaveLength();
	...
	effect->AsShaderResource("StarfieldMap")->SetResource(starField->SRV());

	if (loadMoonMap == L"")
		moon = new Texture(Textures + L"moon.tga");
	else
		moon = new Texture(loadMoonMap);
	effect->AsShaderResource("MoonMap")->SetResource(moon->SRV());

	if (loadGlowMap == L"")
		moonGlow = new Texture(Textures + L"moonglow.tga");
	else
		moonGlow = new Texture(loadGlowMap);
	effect->AsShaderResource("MoonGlowMap")->SetResource(moonGlow->SRV());
	...
	effect->AsShaderResource("CloudMap")->SetResource(cloud->SRV());
}
...
void ScatterSky::Update()
{
	...
	//shader->AsVector("SunColor")->SetFloatVector(sunColor);
...
void ScatterSky::Render()
{
	...
	// Moon
	{
		...
		effect->AsScalar("MoonAlpha")->SetFloat(MoonAlpha);
...

		{
			...
			effect->AsMatrix("World")->SetMatrix(matF);
