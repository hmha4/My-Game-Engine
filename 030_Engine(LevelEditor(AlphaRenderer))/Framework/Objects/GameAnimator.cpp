...
void GameAnimator::AddClip(UINT clipIndex, wstring clipFile)
{
	...
	string clipName = String::ToString(fileName);
	clipNames.push_back(clipName);
	clips[clipName] = new ModelClip(clipFile);
	clips[clipName]->ClipIndex() = clipIndex;
	clips[clipName]->AnimName() = fileName;

	instance->AddClip(clips[clipName]);
}
