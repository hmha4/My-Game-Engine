...
void Effect::Pass::Dispatch(UINT x, UINT y, UINT z)
{
	...
	D3D::GetDC()->CSSetShaderResources(0, 16, null);

	ID3D11UnorderedAccessView* nullUav[16] = { 0 };
	D3D::GetDC()->CSSetUnorderedAccessViews(0, 16, nullUav, NULL);
...
{
	//return effect->GetVariableByName(name.c_str())->AsConstantBuffer();
