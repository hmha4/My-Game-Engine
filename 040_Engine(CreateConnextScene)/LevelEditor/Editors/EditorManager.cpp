...
void EditorManager::Save()
{
	...
	for (; objIter != gameObjects.end(); objIter++)
	{
		...
		else if (objIter->second->Tag() == L"NPC")
		{
			spec->NpcSpec.Name = objIter->second->FileName() + L".spec";
			objIter->second->Position(&spec->NpcSpec.Position);
			objIter->second->Rotation(&spec->NpcSpec.Rotation);
			objIter->second->Scale(&spec->NpcSpec.Scale);
		}
