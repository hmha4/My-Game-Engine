	...
	, loadStarMap(L""), loadMoonMap(L""), loadGlowMap(L""), loadcloudMap(L"")
{
	...
	WaveLength = D3DXVECTOR3(0.75f, 0.57f, 0.475f);
}
...
void ScatterSky::Ready()
{
	...
	frustumPlaneVar = effect->AsVector("FrustumPlanes");
...
void ScatterSky::Update()
{
	...
	//	Moon
	{
		...
		D3DXMatrixTranslation(&T2, pos.x, pos.y, pos.z);

		moonWorld = S * RX * RY * RZ * T1 * T2;

		D3DXMatrixScaling(&S, 2, 2, 1);
		glowWorld = S * moonWorld;
	}
	
	//	Cloud
	{
		D3DXMatrixScaling(&S, 900, 900, 1);
		D3DXMatrixTranslation(&T1, 0, 0, -200.0f);
		D3DXMatrixRotationX(&RX, Math::PI * 0.5f);
		D3DXMatrixTranslation(&T2, pos.x, pos.y, pos.z);
		cloudWorld = S * T1 * RX * T2;
	}
	...
	if (timeFactor > 0.0f)
	{
		...
		Context::Get()->GetGlobalLight()->Ambient = sunColor * 0.5f;
		Context::Get()->GetGlobalLight()->Ambient.a = 1.0f;
		Context::Get()->GetGlobalLight()->Specular = sunColor * 0.89f;
		Context::Get()->GetGlobalLight()->Specular.a = 16.0f;
...
void ScatterSky::Render()
{
	...
	worldVariable->SetMatrix(skyWorld);
	...
	// Moon
	{
...

		{
			...
			worldVariable->SetMatrix(moonWorld);
...

		{
			...
			worldVariable->SetMatrix(glowWorld);
	...
	// Cloud
	{
		...
		worldVariable->SetMatrix(cloudWorld);
...
void ScatterSky::RenderReflection(D3DXMATRIX reflectionView)
{
	effect->AsMatrix("ReflectionView")->SetMatrix(reflectionView);
	...
	worldVariable->SetMatrix(skyWorld);
	effect->DrawIndexed(1, 0, indexCount);
...
void ScatterSky::BindSphereBuffer()
{
	//CreateVertexBuffer
	{
...
void ScatterSky::BindQuadBuffer()
{
	//CreateVertexBuffer
	{
		D3D11_BUFFER_DESC desc = { 0 };
