...
int GameEnemy::ActionIdle()
{
	...
	int rnd = Math::Random(0, 10);
	if (rnd == 0)
	{
		if (SoundManager::Get()->isPlaySound("25EnemyIdle") == false)
			SoundManager::Get()->play("25EnemyIdle", (1.0f - (distance / 50)) * 0.5f);
	}
	...
	if (hit->IsPlay() == true)
	{
		...
		if (k > 10)
		{
			...
			if (result != NULL)
			{
				...
				SoundManager::Get()->play("15SwordHit", 0.4f);
				SoundManager::Get()->play("30EnemyHit", 1.0f);
			}
		}
...
int GameEnemy::NumericAlertDistance()
{
	...
	if (distance <= 30)
	{
		if (SoundManager::Get()->isPlaySound("01GameBGM") == true)
		{
			SoundManager::Get()->stop("01GameBGM");
			SoundManager::Get()->play("02FightBGM");
		}
		if (SoundManager::Get()->isPlaySound("27EnemyAlert") == false)
			SoundManager::Get()->play("27EnemyAlert", 0.5f);
	}
...
int GameEnemy::ActionRun()
{
	...
	if (c == 6)
	{
		if (SoundManager::Get()->isPlaySound("31EnemyFootstep") == true)
			SoundManager::Get()->stop("31EnemyFootstep");
	}
	else if (c == 7)
	{
		if (SoundManager::Get()->isPlaySound("31EnemyFootstep") == false)
			SoundManager::Get()->play("31EnemyFootstep", 0.3f);
	}
	else if (c == 17)
	{
		if (SoundManager::Get()->isPlaySound("31EnemyFootstep") == true)
...
int GameEnemy::ActionAttack()
{
	...
	if (attack->IsPlay() == false)
	{
		...
		SoundManager::Get()->play("29EnemyAttack");
		return true;
	}
...
int GameEnemy::ActionScreamNTurn()
{
	...
	if (alert->IsPlay() == false)
	{
		...
		SoundManager::Get()->play("26EnemyScream", 1.0f);
...
int GameEnemy::ActionDie()
{
	...
	if (die->IsPlay() == false)
	{
		...
		UIManager::Get()->GetRender2D(L"08BarFront" + to_wstring(model->InstanceID()))->SetUV(L"x", (float)Life(), 10.0f);
		SoundManager::Get()->play("28EnemyDie");
		return true;
	}
