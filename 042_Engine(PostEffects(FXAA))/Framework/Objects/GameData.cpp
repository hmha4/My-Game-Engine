...
GameDataSpec * GameDataSpecManager::LoadTerrain(wstring file)
{
	...
	matNode = matNode->NextSiblingElement();
	node = matNode->FirstChildElement();
	spec->FlowerCount = (UINT)node->FloatText();

	Xml::XMLElement * flower = NULL;
	spec->flowerDesc.resize(spec->FlowerCount);
	for (UINT i = 0; i < spec->FlowerCount; i++)
	{
		node = node->NextSiblingElement();
		flower = node->FirstChildElement();

		spec->flowerDesc[i].TextureNumber = (UINT)flower->FloatText();
		flower = flower->NextSiblingElement();

		spec->flowerDesc[i].FlowerNum = (UINT)flower->FloatText();
		flower = flower->NextSiblingElement();

		spec->flowerDesc[i].FlowerSize = (UINT)flower->FloatText();
		flower = flower->NextSiblingElement();

		spec->flowerDesc[i].NumOfRoots = (UINT)flower->FloatText();
		flower = flower->NextSiblingElement();

		Xml::XMLElement * position = flower->FirstChildElement();
		spec->flowerDesc[i].FlowerPosition.x = position->FloatText();
		position = position->NextSiblingElement();
		spec->flowerDesc[i].FlowerPosition.y = position->FloatText();
		position = position->NextSiblingElement();
		spec->flowerDesc[i].FlowerPosition.z = position->FloatText();
		position = position->NextSiblingElement();
	}
...
void GameDataSpecManager::WriteTerrain(wstring fileName, GameTerrainSpec * spec)
{
	...
	node = document->NewElement("FlowerDesc");
	root->LinkEndChild(node);

	element = document->NewElement("FlowerCount");
	element->SetText(spec->FlowerCount);
	node->LinkEndChild(element);

	Xml::XMLElement * flower = NULL;
	for (UINT i = 0; i < spec->FlowerCount; i++)
	{
		element = document->NewElement(string("Flower").append(to_string(i)).c_str());
		node->LinkEndChild(element);

		flower = document->NewElement("TextureNumber");
		flower->SetText(spec->flowerDesc[i].TextureNumber);
		element->LinkEndChild(flower);

		flower = document->NewElement("FlowerNum");
		flower->SetText(spec->flowerDesc[i].FlowerNum);
		element->LinkEndChild(flower);

		flower = document->NewElement("FlowerSize");
		flower->SetText(spec->flowerDesc[i].FlowerSize);
		element->LinkEndChild(flower);

		flower = document->NewElement("NumOfRoots");
		flower->SetText(spec->flowerDesc[i].NumOfRoots);
		element->LinkEndChild(flower);

		flower = document->NewElement("FlowerPosition");
		element->LinkEndChild(flower);

		Xml::XMLElement * position = NULL;
		position = document->NewElement("FlowerX");
		position->SetText(spec->flowerDesc[i].FlowerPosition.x);
		flower->LinkEndChild(position);
		position = document->NewElement("FlowerY");
		position->SetText(spec->flowerDesc[i].FlowerPosition.y);
		flower->LinkEndChild(position);
		position = document->NewElement("FlowerZ");
		position->SetText(spec->flowerDesc[i].FlowerPosition.z);
		flower->LinkEndChild(position);
	}
...
GameDataSpec * GameDataSpecManager::LoadScene(wstring file)
{
	...
	node = node->NextSiblingElement();
	Xml::XMLElement * npc = NULL;
	npc = node->FirstChildElement();
	spec->NpcSpec.Name = String::ToWString(npc->GetText());

	npc = npc->NextSiblingElement();

	GetTransforms(&npc, &world);
	GetVectors(&spec->NpcSpec.Scale, &spec->NpcSpec.Rotation, &spec->NpcSpec.Position, world);
...
void GameDataSpecManager::WriteScene(wstring fileName, GameSceneSpec * spec)
{
	...
	element = document->NewElement("NpcSpec");
	node->LinkEndChild(element);

	Xml::XMLElement * npc = NULL;
	npc = document->NewElement("Spec");
	npc->SetText(String::ToString(spec->NpcSpec.Name).c_str());
	element->LinkEndChild(npc);

	world = GetMatrix(spec->NpcSpec.Scale, spec->NpcSpec.Rotation, spec->NpcSpec.Position);
	SetTransforms(npc, element, document, world);
