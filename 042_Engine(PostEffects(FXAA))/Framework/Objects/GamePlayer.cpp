	...
	, isTalk(false)
{
	...
	model->Play("Sitting Idle", true, 0.2f);
...
void GamePlayer::IsTalk(bool val) 
{
	isTalk = val; 
}

void GamePlayer::SetUI(bool val)
{
	UIManager::Get()->SetUiRender(L"01Orb_back", val);
	UIManager::Get()->SetUiRender(L"04base_back", val);
	UIManager::Get()->SetUiRender(L"02Health", val);
	UIManager::Get()->SetUiRender(L"05Stealth", val);
	UIManager::Get()->SetUiRender(L"03Orb_glass", val);
	UIManager::Get()->SetUiRender(L"06base_glass", val);
	UIManager::Get()->SetUiRender(L"10GameSceneUI", val);
	UIManager::Get()->SetUiRender(L"09SkillIcon", val);
	UIManager::Get()->SetUiRender(L"09TextBox", val);
}
...
void GamePlayer::PlaySwordSlash()
{
	int ran1 = Math::Random(16, 20);
	int ran2 = ran1 - 15;
	string playName = to_string(ran1) + "SwordSlash" + to_string(ran2);

	SoundManager::Get()->play(playName, 0.4f);
}
...
{
	if (isTalkable == true) return false;
	...
	case 0:
		...
		if (Keyboard::Get()->Down('1'))
		{
			...
			SoundManager::Get()->play("09Skill1", 0.5f);
			SoundManager::Get()->play("23Skill1Effect", 0.5f);
		}
		...
		else if (Keyboard::Get()->Down('4'))
		{
			...
			SoundManager::Get()->play("10Skill2", 0.3f);
			SoundManager::Get()->play("22Skill2Effect");
		}
		break;
	case 1:
...
int GamePlayer::ActionHit()
{
	...
	if (hit->IsPlay() == false)
	{
		...
		SoundManager::Get()->play("11PlayerHit", 0.5f);
...
int GamePlayer::ActionMove()
{
	...
	if (Keyboard::Get()->Press('W'))
	{
		...
		{
			int c = model->CurrKeyframe();
			if (c == 9)
			{
				if (SoundManager::Get()->isPlaySound("24Footstep") == false)
					SoundManager::Get()->play("24Footstep", 0.3f);
			}
			else if (c == 24)
			{
				if (SoundManager::Get()->isPlaySound("24Footstep") == false)
		...
		else
		{
			...
			if (c == 5)
			{
				if (SoundManager::Get()->isPlaySound("24Footstep") == true)
					SoundManager::Get()->stop("24Footstep");
			}
			else if (c == 6)
			{
				if (SoundManager::Get()->isPlaySound("24Footstep") == false)
		...
		case 0:
			if (attack1->IsPlay() == true)
			{
				...
				if (currKeyframe > 10 && currKeyframe <= 20)
				{
					...
					if (currKeyframe == 11)
					{
						...
						if (SoundManager::Get()->isPlaySound("12Attack1") == false)
							SoundManager::Get()->play("12Attack1", 0.5f);
						PlaySwordSlash();
					}
				}
			...
			else if(attack2->IsPlay() == true)
			{
				...
				else if (currKeyframe > 37 && currKeyframe <= 50)
				{
					...
					if (currKeyframe == 38)
					{
						if (SoundManager::Get()->isPlaySound("13Attack2") == false)
							SoundManager::Get()->play("13Attack2", 0.5f);
						PlaySwordSlash();
					}
				}
				else if (currKeyframe > 50 && currKeyframe <= 69)
				{
					...
					if (currKeyframe == 63)
					{
						if (SoundManager::Get()->isPlaySound("14Attack3") == false)
							SoundManager::Get()->play("14Attack3", 0.5f);
						PlaySwordSlash();
					}
				}
