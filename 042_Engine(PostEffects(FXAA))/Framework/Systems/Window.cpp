...
WPARAM Window::Run(IExecute * main)
{
	...
	GameDataSpecManager::Create();
	InstanceManager::Create();
	AlphaRenderer::Create();
	...
	SoundManager::Delete();
	SceneManager::Delete();
	AlphaRenderer::Delete();
	GameDataSpecManager::Delete();
...
void Window::MainRender()
{
	...
	LightningManager::Get()->PreRender();

	if (DEFERRED == true)
	{
		DeferredRendering::Get()->SetPreRender();
		{
			mainExecute->Render();
		}

		DeferredRendering::Get()->SetRender();
		{
			AlphaRenderer::Get()->RenderF();
			DeferredRendering::Get()->Render();
			AlphaRenderer::Get()->RenderD();
			ParticleManager::Get()->Render();
			LightningManager::Get()->Render();
		}
		D3D::Get()->SetRenderTarget();
		D3D::Get()->Clear(D3DXCOLOR(0.8f, 0.8f, 0.8f, 1));	//	백 버퍼, 깊이/스텐실 버퍼 클리어
		{
			DeferredRendering::Get()->PostRender();
			
			//DirectWrite::GetDC()->BeginDraw();
			//{
			//	//	BackBuffer를 클리어해 주면 이전게 사라짐
			//	//	DirectWrite::GetDC()->Clear();
			LightningManager::Get()->PostRender();
	...
	if (message == WM_SIZE)
	{
		...
		ImGui_ImplDX11_InvalidateDeviceObjects();
		{
			if (mainExecute != NULL)
			{
				...
				//if (DeferredRendering::Get() != NULL)
				//	DeferredRendering::Get()->ResizeScreen(width, height);
			}
		}
		//ImGui::Validate();
...
void Window::ImGuiStyleSetting(bool bStyleDark_, float alpha_)
{
	...
	style.GrabRounding = 6.0f;             // Radius of grabs corners rounding. Set to 0.0f to have rectangular slider grabs.
	...
	style.Colors[ImGuiCol_TabActive] = ImVec4(0.26f, 0.59f, 0.98f, 1.0f);
