...
void GeometryManager::Initialize()
{
	...
	//ScaleModel
	{
		axisModels[3]->xAxisModel = new Geometry_Scale(GizmoGeometry::XYZAxis::XAxis, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		...
		axisModels[3]->yAxisModel = new Geometry_Scale(GizmoGeometry::XYZAxis::YAxis, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		...
		axisModels[3]->zAxisModel = new Geometry_Scale(GizmoGeometry::XYZAxis::ZAxis, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		...
		axisModels[3]->XZPlane = new Geometry_Plane(GizmoGeometry::XYZAxis::XZPlane, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[3]->XZPlane->SetLineLength(lineLength);
		axisModels[3]->XZPlane->SetTargetWorld(&initMat);

		axisModels[3]->XYPlane = new Geometry_Plane(GizmoGeometry::XYZAxis::XYPlane, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[3]->XYPlane->SetLineLength(lineLength);
		axisModels[3]->XYPlane->SetTargetWorld(&initMat);

		axisModels[3]->YZPlane = new Geometry_Plane(GizmoGeometry::XYZAxis::YZPlane, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[3]->YZPlane->SetLineLength(lineLength);
		axisModels[3]->YZPlane->SetTargetWorld(&initMat);

		axisModels[3]->xLine = new Geometry_Line(GizmoGeometry::XYZAxis::XLine, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[3]->xLine->SetLineLength(lineLength);
		axisModels[3]->xLine->SetTargetWorld(&initMat);

		axisModels[3]->yLine = new Geometry_Line(GizmoGeometry::XYZAxis::YLine, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[3]->yLine->SetLineLength(lineLength);
		axisModels[3]->yLine->SetTargetWorld(&initMat);

		axisModels[3]->zLine = new Geometry_Line(GizmoGeometry::XYZAxis::ZLine, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[3]->zLine->SetLineLength(lineLength);
		axisModels[3]->zLine->SetTargetWorld(&initMat);

		axisModels[3]->xBoundingBox = new Geometry_Box(GizmoGeometry::XYZAxis::XBox, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[3]->xBoundingBox->SetLineLength(lineLength);
		axisModels[3]->xBoundingBox->SetTargetWorld(&initMat);

		axisModels[3]->yBoundingBox = new Geometry_Box(GizmoGeometry::XYZAxis::YBox, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[3]->yBoundingBox->SetLineLength(lineLength);
		axisModels[3]->yBoundingBox->SetTargetWorld(&initMat);

		axisModels[3]->zBoundingBox = new Geometry_Box(GizmoGeometry::XYZAxis::ZBox, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
	...
	//ScaleModel
	{
		axisModels[2]->xAxisModel = new Geometry_Scale(GizmoGeometry::XYZAxis::XAxis, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[2]->xAxisModel->SetLineLength(lineLength);
		axisModels[2]->xAxisModel->SetTargetWorld(&initMat);

		axisModels[2]->yAxisModel = new Geometry_Scale(GizmoGeometry::XYZAxis::YAxis, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[2]->yAxisModel->SetLineLength(lineLength);
		axisModels[2]->yAxisModel->SetTargetWorld(&initMat);

		axisModels[2]->zAxisModel = new Geometry_Scale(GizmoGeometry::XYZAxis::ZAxis, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[2]->zAxisModel->SetLineLength(lineLength);
		axisModels[2]->zAxisModel->SetTargetWorld(&initMat);

		axisModels[2]->XZPlane = new Geometry_Plane(GizmoGeometry::XYZAxis::XZPlane, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[2]->XZPlane->SetLineLength(lineLength);
		axisModels[2]->XZPlane->SetTargetWorld(&initMat);

		axisModels[2]->XYPlane = new Geometry_Plane(GizmoGeometry::XYZAxis::XYPlane, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[2]->XYPlane->SetLineLength(lineLength);
		axisModels[2]->XYPlane->SetTargetWorld(&initMat);

		axisModels[2]->YZPlane = new Geometry_Plane(GizmoGeometry::XYZAxis::YZPlane, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[2]->YZPlane->SetLineLength(lineLength);
		axisModels[2]->YZPlane->SetTargetWorld(&initMat);

		axisModels[2]->xLine = new Geometry_Line(GizmoGeometry::XYZAxis::XLine, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[2]->xLine->SetLineLength(lineLength);
		axisModels[2]->xLine->SetTargetWorld(&initMat);

		axisModels[2]->yLine = new Geometry_Line(GizmoGeometry::XYZAxis::YLine, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[2]->yLine->SetLineLength(lineLength);
		axisModels[2]->yLine->SetTargetWorld(&initMat);

		axisModels[2]->zLine = new Geometry_Line(GizmoGeometry::XYZAxis::ZLine, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[2]->zLine->SetLineLength(lineLength);
		axisModels[2]->zLine->SetTargetWorld(&initMat);

		axisModels[2]->xBoundingBox = new Geometry_Box(GizmoGeometry::XYZAxis::XBox, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[2]->xBoundingBox->SetLineLength(lineLength);
		axisModels[2]->xBoundingBox->SetTargetWorld(&initMat);

		axisModels[2]->yBoundingBox = new Geometry_Box(GizmoGeometry::XYZAxis::YBox, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[2]->yBoundingBox->SetLineLength(lineLength);
		axisModels[2]->yBoundingBox->SetTargetWorld(&initMat);

		axisModels[2]->zBoundingBox = new Geometry_Box(GizmoGeometry::XYZAxis::ZBox, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
	...
	//RotateModel
	{
		axisModels[1]->xAxisModel = new Geometry_Rotate(GizmoGeometry::XYZAxis::XAxis, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[1]->xAxisModel->SetLineLength(lineLength);
		axisModels[1]->xAxisModel->SetTargetWorld(&initMat);

		axisModels[1]->yAxisModel = new Geometry_Rotate(GizmoGeometry::XYZAxis::YAxis, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[1]->yAxisModel->SetLineLength(lineLength);
		axisModels[1]->yAxisModel->SetTargetWorld(&initMat);

		axisModels[1]->zAxisModel = new Geometry_Rotate(GizmoGeometry::XYZAxis::ZAxis, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[1]->zAxisModel->SetLineLength(lineLength);
		axisModels[1]->zAxisModel->SetTargetWorld(&initMat);

		axisModels[1]->XZPlane = new Geometry_Plane(GizmoGeometry::XYZAxis::XZPlane, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[1]->XZPlane->SetLineLength(lineLength);
		axisModels[1]->XZPlane->SetTargetWorld(&initMat);

		axisModels[1]->XYPlane = new Geometry_Plane(GizmoGeometry::XYZAxis::XYPlane, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[1]->XYPlane->SetLineLength(lineLength);
		axisModels[1]->XYPlane->SetTargetWorld(&initMat);

		axisModels[1]->YZPlane = new Geometry_Plane(GizmoGeometry::XYZAxis::YZPlane, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[1]->YZPlane->SetLineLength(lineLength);
		axisModels[1]->YZPlane->SetTargetWorld(&initMat);

		axisModels[1]->xLine = new Geometry_Line(GizmoGeometry::XYZAxis::XLine, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[1]->xLine->SetLineLength(lineLength);
		axisModels[1]->xLine->SetTargetWorld(&initMat);

		axisModels[1]->yLine = new Geometry_Line(GizmoGeometry::XYZAxis::YLine, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[1]->yLine->SetLineLength(lineLength);
		axisModels[1]->yLine->SetTargetWorld(&initMat);

		axisModels[1]->zLine = new Geometry_Line(GizmoGeometry::XYZAxis::ZLine, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[1]->zLine->SetLineLength(lineLength);
		axisModels[1]->zLine->SetTargetWorld(&initMat);

		axisModels[1]->xBoundingBox = new Geometry_Box(GizmoGeometry::XYZAxis::XBox, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[1]->xBoundingBox->SetLineLength(lineLength);
		axisModels[1]->xBoundingBox->SetTargetWorld(&initMat);

		axisModels[1]->yBoundingBox = new Geometry_Box(GizmoGeometry::XYZAxis::YBox, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[1]->yBoundingBox->SetLineLength(lineLength);
		axisModels[1]->yBoundingBox->SetTargetWorld(&initMat);

		axisModels[1]->zBoundingBox = new Geometry_Box(GizmoGeometry::XYZAxis::ZBox, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
	...
	//TransModel
	{
		axisModels[0]->xAxisModel = new Geometry_Trans(GizmoGeometry::XYZAxis::XAxis, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[0]->xAxisModel->SetLineLength(lineLength);
		axisModels[0]->xAxisModel->SetTargetWorld(&initMat);

		axisModels[0]->yAxisModel = new Geometry_Trans(GizmoGeometry::XYZAxis::YAxis, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[0]->yAxisModel->SetLineLength(lineLength);
		axisModels[0]->yAxisModel->SetTargetWorld(&initMat);

		axisModels[0]->zAxisModel = new Geometry_Trans(GizmoGeometry::XYZAxis::ZAxis, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[0]->zAxisModel->SetLineLength(lineLength);
		axisModels[0]->zAxisModel->SetTargetWorld(&initMat);

		axisModels[0]->XZPlane = new Geometry_Plane(GizmoGeometry::XYZAxis::XZPlane, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[0]->XZPlane->SetLineLength(lineLength);
		axisModels[0]->XZPlane->SetTargetWorld(&initMat);

		axisModels[0]->XYPlane = new Geometry_Plane(GizmoGeometry::XYZAxis::XYPlane, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[0]->XYPlane->SetLineLength(lineLength);
		axisModels[0]->XYPlane->SetTargetWorld(&initMat);

		axisModels[0]->YZPlane = new Geometry_Plane(GizmoGeometry::XYZAxis::YZPlane, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[0]->YZPlane->SetLineLength(lineLength);
		axisModels[0]->YZPlane->SetTargetWorld(&initMat);

		axisModels[0]->xLine = new Geometry_Line(GizmoGeometry::XYZAxis::XLine, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[0]->xLine->SetLineLength(lineLength);
		axisModels[0]->xLine->SetTargetWorld(&initMat);

		axisModels[0]->yLine = new Geometry_Line(GizmoGeometry::XYZAxis::YLine, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[0]->yLine->SetLineLength(lineLength);
		axisModels[0]->yLine->SetTargetWorld(&initMat);

		axisModels[0]->zLine = new Geometry_Line(GizmoGeometry::XYZAxis::ZLine, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[0]->zLine->SetLineLength(lineLength);
		axisModels[0]->zLine->SetTargetWorld(&initMat);

		axisModels[0]->xBoundingBox = new Geometry_Box(GizmoGeometry::XYZAxis::XBox, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[0]->xBoundingBox->SetLineLength(lineLength);
		axisModels[0]->xBoundingBox->SetTargetWorld(&initMat);

		axisModels[0]->yBoundingBox = new Geometry_Box(GizmoGeometry::XYZAxis::YBox, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
		axisModels[0]->yBoundingBox->SetLineLength(lineLength);
		axisModels[0]->yBoundingBox->SetTargetWorld(&initMat);

		axisModels[0]->zBoundingBox = new Geometry_Box(GizmoGeometry::XYZAxis::ZBox, gizmoEffect, lineEffect, GizmoGeometry::GizmoType::Component);
