...
//===========================Geometry=========================//
GizmoGeometry::GizmoGeometry(XYZAxis axis, Effect*effect, Effect*line, int TransModel)
	:effect(NULL), vertexCount(0), indicesCount(0),
...
void GizmoGeometry::Initialize()
{
}

void GizmoGeometry::Ready()
{
	line = new LineMaker(this->lineEffect);
...
D3DXVECTOR3 GizmoGeometry::Position()
{
	return position;
}

void GizmoGeometry::Position(D3DXVECTOR3 pos)
{
	position = pos;
	UpdateWorld();
}

void GizmoGeometry::Position(float x, float y, float z)
{
	Position(D3DXVECTOR3(x, y, z));
	UpdateWorld();
}

D3DXVECTOR3 GizmoGeometry::Rotation()
{
	return rotate;
}

void GizmoGeometry::Rotation(D3DXVECTOR3 rotate)
{
	this->rotate = rotate;
	UpdateWorld();
}

void GizmoGeometry::Rotation(float x, float y, float z)
{
	Rotation(D3DXVECTOR3(x, y, z));
	UpdateWorld();
}

void GizmoGeometry::RotationDegree(D3DXVECTOR3 rotate)
{
	Rotation(Math::ToRadian(rotate.x), Math::ToRadian(rotate.y), Math::ToRadian(rotate.z));
	UpdateWorld();
}

void GizmoGeometry::RotationDegree(float x, float y, float z)
{
	Rotation(Math::ToRadian(x), Math::ToRadian(y), Math::ToRadian(z));
	UpdateWorld();
}

D3DXVECTOR3 GizmoGeometry::Scale()
{
	return scale;
}

void GizmoGeometry::Scale(D3DXVECTOR3 scale)
{
	this->scale = scale;
	UpdateWorld();
}

void GizmoGeometry::Scale(float x, float y, float z)
{
	Scale(D3DXVECTOR3(x, y, z));
	UpdateWorld();
}

void GizmoGeometry::SetColor(float x, float y, float z)
{
	gizmoColor = { x,y,z,1 };
	line->SetColor(D3DXCOLOR(x, y, z, 1));
}

void GizmoGeometry::SetColor(D3DXCOLOR color)
{
	gizmoColor = color;
	line->SetColor(color);
}

void GizmoGeometry::SetColor(D3DXVECTOR3 color)
{
	gizmoColor = { color.x,color.y,color.z,1 };
	line->SetColor(D3DXCOLOR(color.x, color.y, color.z, 1));
}

void GizmoGeometry::UpdateOnColor()
{
	switch (axisType)
	{
	case GizmoGeometry::XYZAxis::XLine:
		break;
	case GizmoGeometry::XYZAxis::YLine:
		break;
	case GizmoGeometry::XYZAxis::ZLine:
		break;
	case GizmoGeometry::XYZAxis::XAxis:
	case GizmoGeometry::XYZAxis::YAxis:
	case GizmoGeometry::XYZAxis::ZAxis:
		gizmoColor = { 1,1,0,1 };
		line->SetColor(D3DXCOLOR(1, 1, 0, 1));
		break;
	case GizmoGeometry::XYZAxis::XZPlane:
	case GizmoGeometry::XYZAxis::XYPlane:
	case GizmoGeometry::XYZAxis::YZPlane:
		gizmoColor = { 1,1,0,0.7f };
		break;
	}
}

void GizmoGeometry::UpdateOffColor()
{
	switch (axisType)
	{
	case GizmoGeometry::XYZAxis::XLine:
...
void GizmoGeometry::SetLineLength(float length)
{
	lineLength = length;
}

void GizmoGeometry::SetTargetWorld(D3DXMATRIX * target)
{
	targetWorld = target;
}

bool GizmoGeometry::IsPicked()
{
	D3DXVECTOR3 start;
...
void GizmoGeometry::UpdateWorld()
{
	switch (gizmoType)
	{
	case GizmoGeometry::GizmoType::Component:
	{
		//넘어온월드값 분해해서주든해야함
	...
	break;
	case GizmoGeometry::GizmoType::Own:
	{
		D3DXMATRIX S, R, T;
		D3DXMatrixScaling(&S, scale.x, scale.y, scale.z);
...
void GizmoGeometry::Render()
{
	...
	if (axisType == XYZAxis::XAxis || axisType == XYZAxis::YAxis || axisType == XYZAxis::ZAxis)
	{
		...
		lineColorVar->SetFloatVector(gizmoColor);

		effect->DrawIndexed(0, 0, indicesCount);
		line->DrawLine(1);
	}
	...
	else if (axisType == XYZAxis::YZPlane || axisType == XYZAxis::XYPlane || axisType == XYZAxis::XZPlane) // 면일때
	{
		...
		effect->DrawIndexed(0, 1, indicesCount);
	}
}
//===========================Geometry=========================//

//===========================Geometry_Scale=========================//
Geometry_Scale::Geometry_Scale(XYZAxis axis, Effect*effect, Effect*line, GizmoType gizmoType)
	:GizmoGeometry(axis, effect, line)
{
...
//===========================Geometry_Rotate=========================//
Geometry_Rotate::Geometry_Rotate(XYZAxis axis, Effect*effect, Effect*line, GizmoType gizmoType)
	:GizmoGeometry(axis, effect, line)
{
	geoType = GeometryType::AXIS_TYPE;
	this->gizmoType = gizmoType;
...
//===========================Geometry_Trans=========================//
Geometry_Trans::Geometry_Trans(XYZAxis axis, Effect*effect, Effect*line, GizmoType gizmoType)
	:GizmoGeometry(axis, effect, line)
{
	geoType = GeometryType::AXIS_TYPE;
	this->gizmoType = gizmoType;
...
Geometry_Plane::Geometry_Plane(XYZAxis axis, Effect*effect, Effect*line, GizmoType gizmoType)
	:GizmoGeometry(axis, effect, line, 0)
{
...
Geometry_Line::Geometry_Line(XYZAxis axis, Effect*effect, Effect*line, GizmoType gizmoType)
	:GizmoGeometry(axis, effect, line, 0)
{
	geoType = GeometryType::GEO_TYPE;
	this->gizmoType = gizmoType;
...
Geometry_Box::Geometry_Box(XYZAxis axis, Effect*effect, Effect*line, GizmoType gizmoType)
	:GizmoGeometry(axis, effect, line, 0)
{
	geoType = GeometryType::GEO_TYPE;
	this->gizmoType = gizmoType;
