	...
	case GameNPC::STATE::IDLE:
		if (player->IsTalk() == true)
		{
			...
			if (Mouse::Get()->Down(0))
			{
				...
				if (uiIndex > 4)
				{
					...
					state = STATE::IDLE2;
					uiIndex--;
					SoundManager::Get()->play("03Quest", 1.0f);
				}
			}
		...
		if (player->IsTalkable() == true)
		{
			...
			if (Mouse::Get()->Down(0))
			{
				...
				SoundManager::Get()->play("04FemaleTalk1", 1.0f);
			...
			if (enemyCount <= 0)
			{
				if (Mouse::Get()->Down(0))
				{
					...
					if (uiIndex > 6)
					{
						...
						SoundManager::Get()->play("07FemaleTalk4", 1.0f);
					}
				}
			}
			else
		...
		if (player->IsTalkable() == true)
		{
			...
			if (Mouse::Get()->Down(0))
			{
				...
				if (enemyCount > 0)
				{
					dialog13 = !dialog13;
					UIManager::Get()->SetUiRender(L"13Dialog", dialog13);
					SoundManager::Get()->play("05FemaleTalk2", 1.0f);
				}
				else
				{
					UIManager::Get()->SetUiRender(L"12Dialog" + to_wstring(uiIndex++), true);
					SoundManager::Get()->play("06FemaleTalk3", 1.0f);
				}
...
void GameNPC::Render()
{
	...
	SoundManager::Get()->SetVolume("21CampFire", (1.0f - (distance / 30)) * 0.3f);
